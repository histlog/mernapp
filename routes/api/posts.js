...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    // Check user to ensure he deletes only his post
...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    // Check if post is already liked by this user, if ===0, then not liked yet.
    if (
    ...
    const removeIndex = post.like
      ...
      .indexOf(req.user.id);
    // Use index to splice out the entry from like array
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    // Check if user deleting is guy who made it
